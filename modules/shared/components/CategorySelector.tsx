...
export function CategorySelector() {
  ...
  const categories = ['Tech', 'Sport', 'Gallery', 'Education', 'Local'];
  return (
    <div
              ...
              <Menu.Item key={category}>
                {({ active }) => (
                  <button
                    type="button"
                    className={`${
                      active ? 'bg-lava-red text-white bg-opacity-60' : 'text-white'
                    } group border-none   flex w-full items-center rounded-md px-2 py-2 text-sm`}
                  >
                    {category}
                  </button>
                )}
